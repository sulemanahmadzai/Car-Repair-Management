...
export default function EditStaffPage({
  ...
  const { data: staffList, error: fetchError } = useSWR("/api/staff", fetcher);
  ...
  useEffect(() => {
    if (staffMember) {
      setFormData({
        ...
        lastPaymentDate: staffMember.lastPaymentDate || "",
        tasksCompleted: staffMember.tasksCompleted?.toString() || "0",
        emergencyContactName: staffMember.emergencyContactName || "",
        emergencyContactPhone: staffMember.emergencyContactPhone || "",
        relationship: staffMember.relationship || "",
      });
    }
  }, [staffMember]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/dashboard/staff/${resolvedParams.id}`);
    } catch (err: any) {
